  ...
  createOrUpdate(contentRecord: any) {
    return new Observable((observer) => {
      forkJoin(
        ...
        forkJoin(
          this.updateContentRecordsToSync(
            this.addOrReplaceContentRecord(contentRecordsToSync, contentRecord),
          ),
          this.updateContentRecords(
            this.addOrReplaceContentRecord(contentRecords, contentRecord),
          ),
        ).subscribe(() => {
  ...
  trySynchronizeContentRecords(records: any[]) {
    if (this.connectionService.isOnline) {
      this.authService.currentUser().subscribe((user) => {
        this.contentRecordsSynchronizer
          .sync(records, user['teacher_id'])
          .subscribe();
      });
    }
  }
