...
export class DailyFrequencyService {
  ...
  getOfflineStudents(params: any): Observable<any> {
    ...
    if (params.disciplineId) {
      return this.getOfflineStudentsDisciplineAbsence(
        params.classroomId,
        params.unityId,
        params.disciplineId,
        splitedClassNumbers,
        params.frequencyDate,
      );
    } else {
      return this.getOfflineStudentsGlobalAbsence(
        params.classroomId,
        params.unityId,
        params.disciplineId,
        params.frequencyDate,
  ...
  private getOfflineStudentsGlobalAbsence(
    classroomId: number,
    unityId: number,
    disciplineId: number,
    ...
    return new Observable((observer) => {
      forkJoin([
        ...
        let filteredDailyFrequencies = dailyFrequencies.filter(
          (dailyFrequency: {
            classroom_id: number;
        ...
        if (filteredDailyFrequencies.length === 0) {
          ...
          filteredDailyFrequencies =
            filteredDailyFrequencies.concat(newFrequencies);
          this.saveOfflineFrequencies(dailyFrequencies, newFrequencies);
          this.saveOfflineFrequenciesToSync(dailyFrequenciesToSync, [
            newFrequencies,
          ]);
        }
  ...
  private getOfflineStudentsDisciplineAbsence(
    classroomId: number,
    unityId: number,
    disciplineId: number,
    splitedClassNumbers: string[],
    frequencyDate: string,
  ): Observable<any> {
    return new Observable((observer) => {
      forkJoin([
        from(this.storage.get('frequencies')),
        from(this.storage.get('dailyFrequenciesToSync')),
        this.studentsService.getOfflineDisciplineStudents(
          classroomId,
          disciplineId,
        ),
        this.offlineClassroomFinder.find(classroomId),
        ...
        if (filteredDailyFrequencies.length === 0) {
          ...
          filteredDailyFrequencies =
            filteredDailyFrequencies.concat(newFrequencies);
          this.saveOfflineFrequencies(dailyFrequencies, newFrequencies);
          this.saveOfflineFrequenciesToSync(
            dailyFrequenciesToSync,
            newFrequencies,
          );
        } else if (
          filteredDailyFrequencies.length < splitedClassNumbers.length
        ) {
          const frequencyClasses = filteredDailyFrequencies.map(
            (frequency: { class_number: any }) =>
              String(frequency.class_number),
          );
          const missingClasses = splitedClassNumbers.filter(
            (classNumber) => !frequencyClasses.includes(classNumber),
          );
        ...
        filteredDailyFrequencies = filteredDailyFrequencies.sort(
          this.byClassNumber,
        );
  ...
  private saveOfflineFrequenciesToSync(
    dailyFrequenciesToSync: any[],
    newFrequencies: any[],
  ): void {
  ...
  private saveOfflineFrequencies(
    existingFrequencies: any[],
    newFrequencies: any[],
  ): void {
  ...
  private createOfflineGlobalFrequencies(params: any): any {
    const students = params.students.data.classroom_students.map(
      (element: any) => {
        return {
          active: true,
